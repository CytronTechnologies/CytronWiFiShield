...
ESP8266Client ESP8266Server::available(uint8_t wait)
{
	...
	/*
	if (wifi.readForResponse(",CONNECT", wait) > 0)
	{
		char * p = wifi.searchBuffer(",CONNECT");
		p -= 1;
	...
	if (wifi.updateStatus())
	{
		for (int sock=0; sock<ESP8266_MAX_SOCK_NUM; sock++)
		{
			if ((wifi._status.ipstatus[sock].linkID != 255) &&
			      (wifi._status.ipstatus[sock].tetype == ESP8266_SERVER))
			{
				ESP8266Client client(sock);
...
//TODO: Think about giving server.on and handleclients functions
/*
